...
for test_loader in test_loaders:
    ...
    kernel_dir = os.path.join(opt["path"]["results_root"], 'kernel')
    util.mkdir(dataset_dir)
    util.mkdir(kernel_dir)
    ...
    for test_data in test_loader:
        ...
        #### input dataset_LQ
        model.feed_data(LR_img=test_data["LQ"], GT_img=test_data["GT"])
        model.test()
        visuals = model.get_current_visuals()
        SR_img = visuals["Batch_SR"]
        sr_img = util.tensor2img(visuals["SR"])  # uint8
        # est_sig = visuals["sigma"]
        ...
        # save_ker_path = os.path.join('/home/customer/Desktop/kernel', img_name + ".png")
        util.save_img(sr_img, save_img_path)
        # plt.imsave(save_ker_path, ker_img)
