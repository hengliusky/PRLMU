...
class B_Model(BaseModel):
    def __init__(self, opt):
        ...
        if self.is_train:
            ...
            self.cri_mse = nn.MSELoss().to(self.device)
            self.cri_cro = nn.CrossEntropyLoss().to(self.device)
            # optimizers
    ...
    def feed_data(self, LR_img, label_sig=None, label_size=None, GT_img=None):
        self.var_L = LR_img.to(self.device)

        if not (label_sig is None):
            self.label_sig = label_sig.to(self.device)

        if not (label_size is None):
            self.label_size = label_size.to(self.device)
    ...
    def test(self):
        self.netG.eval()
        with torch.no_grad():
            sr, lr_cyc, est_sig, est_size = self.netG(self.var_L)
            self.fake_SR = sr
            self.est_sig = est_sig
